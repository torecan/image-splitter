...
void NorthDivider(IplImage*img) {
	...
	cvResetImageROI(imgnord);
	cvResetImageROI(img);


	sprintf(outname, "%s//%s", dirname, "nord");
	sprintf(outname, "%s//%s", outname, filename);
	sprintf(outname, "%s%s", outname, ".png");


	cvSaveImage(outname, imgnord);
...
void distributer(IplImage *image) {
	...
	// NORD WILL BE ADDED  1184x192px 
	if (width == 1184 && height == 192) {		// If the resolation is 2336 x 192 handle it as A4
		NorthDivider(image);
	}
...
int main(int argc, char** argv)
{
	...
	while (1) {
		...
		imgnord = cvCreateImage(cvSize(1920, 1080), img->depth, 3);
